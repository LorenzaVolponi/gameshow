...
function normalizeFetchHeaders(request) {
  ...
  headers.get = name => lookup.get(String(name).toLowerCase());
  headers.has = name => lookup.has(String(name).toLowerCase());

  return headers;
}

function normalizeRequest(req) {
  if (typeof Request === 'function' && req instanceof Request) {
    const normalized = {
      method: req.method,
      url: req.url,
      headers: normalizeFetchHeaders(req),
      body: req.body,
      originalRequest: req
    };

    if (typeof req.text === 'function') {
      normalized.text = req.text.bind(req);
    }

    if (typeof req.json === 'function') {
      normalized.json = req.json.bind(req);
    }

    if (typeof req.arrayBuffer === 'function') {
      normalized.arrayBuffer = req.arrayBuffer.bind(req);
    }

    const iterator =
      typeof req[Symbol.asyncIterator] === 'function'
        ? req[Symbol.asyncIterator].bind(req)
        : req.body && typeof req.body[Symbol.asyncIterator] === 'function'
          ? req.body[Symbol.asyncIterator].bind(req.body)
          : null;

    if (iterator) {
      normalized[Symbol.asyncIterator] = iterator;
    }
...
export function createApiHandler(handler) {
  return async function apiHandler(req, res) {
    ...
    const result = await handler(normalizedReq, edgeRes);
